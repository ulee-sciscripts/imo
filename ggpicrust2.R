...
abundance_file <- "pathway_abundance.biom.tsv"
...
ka_nat <- kegg_abundance[,c("pathway", ident_nat$`sample-id`)]
ka_urb <- kegg_abundance[,c("pathway", ident_urb$`sample-id`)]
ka_ster <- kegg_abundance[,c("pathway", ident_ster$`sample-id`)]
ka_nosyn <- kegg_abundance[,c("pathway", ident_nosyn$`sample-id`)]
...
daa_results_df <- pathway_daa(abundance = kegg_abundance %>% column_to_rownames("pathway"), metadata = metadata1[ident_nosyn$`sample-id`,], group = "imo_stage", daa_method = "LinDA", select = NULL, reference = "soil")

daa_results_df_nat <- pathway_daa(abundance = ka_nat %>% column_to_rownames("pathway"), metadata = metadata1_nat, group = "imo_stage", daa_method = "LinDA", select = NULL, reference = "soil")
daa_results_df_urb <- pathway_daa(abundance = ka_urb %>% column_to_rownames("pathway"), metadata = metadata1_urb, group = "imo_stage", daa_method = "LinDA", select = NULL, reference = "soil")
daa_results_df_ster <- pathway_daa(abundance = ka_ster %>% column_to_rownames("pathway"), metadata = metadata1_ster, group = "imo_stage", daa_method = "LinDA", select = NULL, reference = "soil")
daa_results_df_nosyn <- pathway_daa(abundance = ka_nosyn %>% column_to_rownames("pathway"), metadata = metadata1_nosyn, group = "imo_stage", daa_method = "LinDA", select = NULL, reference = "soil")
...
daa_annotated_sub_nat <- pathway_annotation(pathway = "MetaCyc", daa_results_df = daa_sub_nat, ko_to_kegg = FALSE)
daa_annotated_sub_urb <- pathway_annotation(pathway = "MetaCyc", daa_results_df = daa_sub_urb, ko_to_kegg = FALSE)
daa_annotated_sub_ster <- pathway_annotation(pathway = "MetaCyc", daa_results_df = daa_sub_ster, ko_to_kegg = FALSE)
daa_annotated_sub_nosyn <- pathway_annotation(pathway = "MetaCyc", daa_results_df = daa_sub_nosyn, ko_to_kegg = FALSE)
...
daa_annot_ret_nosyn <- daa_annotated_sub_nosyn
daa_annot_ret_nat <- daa_annotated_sub_nat
daa_annot_ret_urb <- daa_annotated_sub_urb
daa_annot_ret_ster <- daa_annotated_sub_ster
...
daa_annot_ret_nosyn_imo <- daa_annot_ret_nosyn[daa_annot_ret_nosyn$group2 == "imo1",]

daa_annot_ret_nosyn_imo$p_adjust <- round(daa_annot_ret_nosyn_imo$p_adjust,5)

sig_nosyn <- daa_annot_ret_nosyn_imo[daa_annot_ret_nosyn_imo$p_adjust < 0.05, "feature"]
low_p_feature_nosyn <- daa_annot_ret_nosyn_imo[order(daa_annot_ret_nosyn_imo$p_adjust), ]$feature[1:20]



pathway_errorbar(
  abundance = ka_nosyn %>% column_to_rownames("pathway"),
  daa_results_df = daa_annot_ret_nosyn[daa_annot_ret_nosyn$group2 == "imo1",],
  Group = metadata1_nosyn$imo_stage,
  select = low_p_feature_nosyn,
  order = "group",
  ko_to_kegg = FALSE,
  p_value_bar = TRUE,
  colors = NULL,
...
daa_annot_ret_nat_imo <- daa_annot_ret_nat[daa_annot_ret_nat$group2 == "imo1",]


daa_annot_ret_nat_imo$p_adjust <- round(daa_annot_ret_nat_imo$p_adjust,5)

sig_nat <- daa_annot_ret_nat_imo[daa_annot_ret_nat_imo$p_adjust < 0.05, "feature"]
low_p_feature_nat <- daa_annot_ret_nat_imo[order(daa_annot_ret_nat_imo$p_adjust), ]$feature[1:20]



pathway_errorbar(
  abundance = ka_nat %>% column_to_rownames("pathway"),
  daa_results_df = daa_annot_ret_nat[daa_annot_ret_nat$group2 == "imo1",],
  Group = metadata1_nat$imo_stage,
  select = low_p_feature_nat,
  order = "group",
  ko_to_kegg = FALSE,
  p_value_bar = TRUE,
  colors = NULL,
...
daa_annot_ret_urb_imo <- daa_annot_ret_urb[daa_annot_ret_urb$group2 == "imo1",]


daa_annot_ret_urb_imo$p_adjust <- round(daa_annot_ret_urb_imo$p_adjust,5)

sig_urb <- daa_annot_ret_urb_imo[daa_annot_ret_urb_imo$p_adjust < 0.05, "feature"]
low_p_feature_urb <- daa_annot_ret_urb_imo[order(daa_annot_ret_urb_imo$p_adjust), ]$feature[1:20]



pathway_errorbar(
  abundance = ka_urb %>% column_to_rownames("pathway"),
  daa_results_df = daa_annot_ret_urb[daa_annot_ret_urb$group2 == "imo1",],
  Group = metadata1_urb$imo_stage,
  select = low_p_feature_urb,
  order = "group",
  ko_to_kegg = FALSE,
  p_value_bar = TRUE,
  colors = NULL,
...
sig_both <- intersect(sig_urb, sig_nat)

write.table(daa_annot_ret_urb[daa_annot_ret_urb$feature %in% sig_both, c("feature", "description")], file="sig_urb_nat.txt", quote=F, row.names=F, col.names=F, sep="\t")

write.table(daa_annot_ret_nat_imo, file="daa_nat.tsv")

write.table(daa_annot_ret_urb_imo, file="daa_urb.tsv")
